...
import '../models/tasfeer.dart.';
import '../models/tasfeer_author.dart';
...
class ApiServices{
  ...
  Future<List<TasfeerAuthor>> getTasfeerAuthor() async {
    const url = "http://api.quran-tafseer.com/tafseer/";
    final res = await http.get(Uri.parse(url));

    final List<dynamic> data = jsonDecode(utf8.decode(res.bodyBytes));
    final List<TasfeerAuthor> authors = data.map((json) => TasfeerAuthor.fromJson(json)).toList();
    authors.sort((a, b) => a.id!.compareTo(b.id!));
    return authors;
  }

  Future<Tasfeer> getTasfeer(int tasfeerId,int? surahNumber, String ayahNumber) async {
    String url = "http://api.quran-tafseer.com/tafseer/$tasfeerId/$surahNumber/$ayahNumber";
    final res = await http.get(Uri.parse(url));
    final data = jsonDecode(utf8.decode(res.bodyBytes));
    return Tasfeer.fromJson(data);
  }
